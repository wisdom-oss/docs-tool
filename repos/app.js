...
var RepoGroups;
(function (RepoGroups) {
    RepoGroups["SERVICE"] = "service";
    RepoGroups["FRONTEND"] = "frontend";
    RepoGroups["OTHER"] = "other";
...
async function fetchAllMeta() {
    ...
    let reqs = [];
    for (let { name: repoName, default_branch } of listForOrg.data)
        reqs.push((async () => {
            ...
            downloads.push((async () => {
                ...
                for (let entry of Object.values(zipInfo.entries)) {
                    ...
                    if (!(!pathFragments[0] ||
                        ...
                        continue;
                    let localFilePath = path.join(__dirname, "../data/repos", sanitize(name), sanitize(branch), pathFragments.join("/"), fileName);
                    await writeZipContent(localFilePath, fileName, entry);
                    if (fileName.match(/(open)?api\.ya?ml/i))
                        localMeta[name][branch].hasApi = fileName;
                    if (pathFragments[0] === "docs")
                        localMeta[name][branch].hasDocs = true;
                    if (pathFragments[0] === "static_docs")
                        localMeta[name][branch].hasStaticDocs = true;
                    if (fileName.match(/readme/i))
                        localMeta[name][branch].hasReadMe = fileName;
                    if (fileName.match(/sidebar[^/]+json/i))
                        localMeta[name][branch].hasDocsSidebar = fileName;
                }
            })());
    }
...
async function writeZipContent(filePath, fileName, entry) {
    if (fileName.match(/readme/i)) {
        ...
        await fs.writeFile(readmeFilePath, (await entry.text())
            .replaceAll("<br>", "<br/>")
            .replaceAll("<hr>", "<hr/>"));
    }
    else {
...
function buildPluginsConfig(docsMeta) {
        ...
        for (let [branch, meta] of Object.entries(branches)) {
            if (meta.hasReadMe) {
                plugins.push(["@docusaurus/plugin-content-pages", {
                        path: path.join("../data/repos", sanitize(name), sanitize(branch), "readme"),
            ...
            if (meta.hasDocs) {
                plugins.push(["@docusaurus/plugin-content-docs", {
                        path: path.join("../data/repos", sanitize(name), sanitize(branch), "docs"),
            ...
            if (meta.hasApi) {
                plugins.push(["docusaurus-plugin-redoc", {
                        spec: path.join("../data/repos", sanitize(name), sanitize(branch), meta.hasApi),
